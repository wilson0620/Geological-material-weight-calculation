...
def mix(borehole_data): #輸入一組或以上鑽井資料並輸出權重計算成果
    ...
    out_len = math.ceil(max(out_len_cal_list) / grid_size[2])
    ...
    for i in borehole_data: #將每一筆鑽井資料中的每一分段填入空白網格。若垂直網格大小需要改變(!=1)，S跟E應該需要改。
        S = 0
        E = grid_size[2]
        for j in range(0, math.ceil((i[-1][1])/grid_size[2])):
    ...
    for i in out_array: #計算輸出網格的材料權重
        ...
        if len(judge) == 1: #如果佔比最高的材料只有一種，該網格指定為該材料。
            ...
            print('垂向單元計算錯誤，請檢查資料(code非1~4?)')
        
        mixed_material.append(m_info) #決定該網格的材料後，寫入網格資訊
    return mixed_material, out_array
...
'''
for i in target_grid:
    result = mix(grid[i]['borehole_data'])
    grid[i]['out_array'] = result[1]
    grid[i]['mixed_material'] = result[0]
